...
startIndex=1
endIndex=$(expr "$nodeCount" \* "$hostCount")
...
case $executeType in
    ...
    *)
        $OUTPUT "
            ...
            $GREEN ./init_region.sh create [host machine count, default 5] [node count per host, default 7] $TAILS   create the validators and regions.
            $GREEN ./init_region.sh remove $TAILS   remove the validators and regions.
            $GREEN ./init_region.sh  help $TAILS       command list
